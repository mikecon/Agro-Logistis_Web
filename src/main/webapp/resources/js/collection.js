...
(function () {
    ...
    var db = firestore;
    var uid = null;
    var reportCollection = document.getElementById("reportCollection");
    ...
    firebase.auth().onAuthStateChanged(function(user) {
        if (user) {
                    ...
                    querySnapshot.forEach(function(doc) {
                        db.doc(uid + "/cloud/collection/" + doc.id).get().then(documentSnapshot => {
                            ...
                            if( documentSnapshot.get('date').toMillis() >= startDate.getTime() &&
                                ...
                                rowsCollection.push(dataRow)

                                collectionScore+=valueField;
                            }
                        });
                    });
                })
            ...
            reportCollection.addEventListener('click', function (e) {
                var d = new Date();

                var docInfo = {
                    info: {
                        title: 'Αναφορά Εισπράξεων',
                ...
                pdfMake.createPdf(docInfo).download('Αναφορά Εισπράξεων '+months[d.getMonth()]+ ' ' +d.getFullYear()+ '.pdf');
            });
